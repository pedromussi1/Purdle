...
import { DEFAULT_LANGUAGE, type LadderPuzzle } from '../types'

interface Manifest {
  dates: string[]
}

const PUZZLES_BASE = `${import.meta.env.BASE_URL}puzzles/ladder/${DEFAULT_LANGUAGE}/`

export type PuzzleStatus = 'loading' | 'ready' | 'unavailable'

async function fetchPuzzle(date: string): Promise<LadderPuzzle | null> {
  try {
    const r = await fetch(PUZZLES_BASE + date + '.json', { cache: 'no-cache' })
    if (!r.ok) return null
    return (await r.json()) as LadderPuzzle
  } catch {
    return null
  }
}
...
async function resolveTodayPuzzle(): Promise<LadderPuzzle | null> {
...
export function usePuzzle(): { status: PuzzleStatus; puzzle: LadderPuzzle | null } {
...
export function useDatePuzzle(date: string): {
  ...
  const [puzzle, setPuzzleLocal] = useState<LadderPuzzle | null>(null)
  const [status, setStatus] = useState<PuzzleStatus>('loading')
  const setStorePuzzle = useStore((s) => s.setPuzzle)
