...
import { EMPTY_LADDER_STATS, type LadderStatsSnapshot, useStats } from '../stats'
...
function rowToSnapshot(r: StatsRow): LadderStatsSnapshot {
  return {
    played: r.played,
    won: r.won,
...
function snapshotToRow(s: LadderStatsSnapshot, userId: string): StatsRow {
  return {
    user_id: userId,
    played: s.played,
    won: s.won,
...
export function mergeSnapshots(a: LadderStatsSnapshot, b: LadderStatsSnapshot): LadderStatsSnapshot {
  ...
  return {
    ...
    // Sum-style metric: take the max so we don't double-count if both
    // sides recorded overlapping days. Slight undercount in the rare
    // independent-device case, like the other games.
...
async function pullCloud(): Promise<LadderStatsSnapshot | null> {
  ...
  const { data, error } = await supabase
    ...
    .maybeSingle()
  if (error) {
    console.error('ladder stats pull failed', error)
    return null
  }
  return data ? rowToSnapshot(data as StatsRow) : EMPTY_LADDER_STATS
}

async function pushCloud(snapshot: LadderStatsSnapshot): Promise<void> {
  const user = useAuth.getState().user
  if (!supabase || !user) return
  const { error } = await supabase
    .from('ladder_stats')
    .upsert(snapshotToRow(snapshot, user.id), { onConflict: 'user_id' })
  if (error) console.error('ladder stats push failed', error)
}
