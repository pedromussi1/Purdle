...
export function StatsModal({ open, onClose }: Props) {
  ...
  const playAgain = useSynonymyStore((s) => s.playAgain)
  const status = useSynonymyStore((s) => s.status)
  const puzzle = useSynonymyStore((s) => s.puzzle)
  const chainLen = useSynonymyStore((s) => s.chain.length)
  ...
  const canShare = (status === 'won' || status === 'gave-up') && puzzle !== null
