...
export interface WordleStoreOptions {
  // localStorage key. Omit to disable persistence (used for ephemeral
  // archive replays — they shouldn't pollute today's saved game).
  persistKey?: string
  // Whether to record outcomes into the global Stats store. Replays don't
  // count toward the player's streak / distribution.
  trackStats?: boolean
}
...
function winMessage(guessCount: number): string {
  return WIN_MESSAGES[guessCount] ?? 'Solved!'
}

export function createWordleStore({
  persistKey,
  trackStats = false,
}: WordleStoreOptions = {}): WordleStoreHook {
  const factory: StateCreator<WordleState> = (set, get) => ({
    solution: null,
    puzzleDate: todayUTC(),
    guesses: [],
    currentGuess: '',
    ...
    setPuzzle: ({ date, solution }) => {
      ...
      // guesses, but verify they're consistent first. Stale guesses (from a
      // previous build's hardcoded word, say) would otherwise show wrong
      // evaluations on the board.
  ...
  if (persistKey) {
    return create<WordleState>()(
      persist(factory, {
        name: persistKey,
        // Persist gameplay state. solution is excluded — it's refetched from
        // the daily puzzle JSON on every mount; toast and revealingRowIndex
  ...
  return create<WordleState>()(factory)
}
