...
import { GameSurface } from '../games/ladder/GameSurface'
import { createLadderStore } from '../games/ladder/createStore'
import { StoreProvider, useStore } from '../games/ladder/StoreContext'
import { useDatePuzzle } from '../games/ladder/hooks/usePuzzle'
import { loadDictionary } from '../games/purdle/lib/dictionary'
...
export function LadderReplay() {
  ...
  if (!date || !ISO_DATE.test(date)) {
    return <Navigate to="/ladder/archive" replace />
  }
  if (date === todayUTC()) {
    return <Navigate to="/ladder" replace />
  }
  return <ReplayShell key={date} date={date} />
}

function ReplayShell({ date }: { date: string }) {
  const [store] = useState(() => createLadderStore({ trackStats: false }))
  return (
    ...
    return (
      <main className="l-page">
        <div className="puzzle-banner">
          No ladder for {date}.{' '}
          <Link to="/ladder/archive" className="archive-back">
            Back to archive
          </Link>
        </div>
      </main>
